...
COVER_TEMPLATE_PATH = SKILL_DIR / "assets" / "templates" / "cover-3-4.html"
...
def render_page(chrome, tpl, out_path, title, content_html, page_label, bottom_tip,
                highlight, bg, footer, icon_path, avatar_path, font_path, md_css_path='', is_cover=False):
    html = tpl
    
    if is_cover:
        # 封面模式：替换标题、副标题、落款和背景色
        replacements = {
            '{{TITLE}}': title if title else '',
            '{{SUBTITLE}}': content_html,  # 内容作为副标题
...
def main():
    ...
    ap.add_argument('--cover', action='store_true', help='封面模式：使用 cover-3-4 模板，无页码无底部提示')
    args = ap.parse_args()
    ...
    # 封面模式使用 cover-3-4 模板，否则使用 article-3-4
    if args.cover:
        tpl = COVER_TEMPLATE_PATH.read_text(encoding='utf-8')
    else:
    ...
    if args.page_num > 0 and args.out:
        if args.cover:
            ...
            render_page(
                ...
                md_css_path=md_css_path, is_cover=False,
            )
        return
